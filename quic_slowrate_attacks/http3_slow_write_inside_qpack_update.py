...
async def main(addr, con: HttpClient, *args, **kwargs):
    ...
    while True:
        async with h3_request_stream(con) as stream_id:
            ...
            for b in map(bytes, itertools.batched(qpack_data, 2)):
                logging.info(f'Sending QPACK update part "{b.hex()}"')
                h3_con._quic.send_stream_data(h3_con._local_encoder_stream_id, data=b)
                con.transmit()
