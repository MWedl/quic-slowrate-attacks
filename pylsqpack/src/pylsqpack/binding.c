...
{
    void* logger_ctx = NULL;  // stderr
    lsqpack_enc_preinit(&self->enc, logger_ctx);
    return 0;
}
...
Encoder_apply_settings(EncoderObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    void *logger_ctx = NULL;  // stderr
    if (lsqpack_enc_init(&self->enc, logger_ctx, max_table_capacity, max_table_capacity, blocked_streams,
